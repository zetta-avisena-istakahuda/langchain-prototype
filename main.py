...
def initRAG(vector_store):
 if 'code_executed' not in st.session_state:
  ...
  condense_q_system_prompt = """You are an assistant for question-answering tasks. Use the following pieces of retrieved context to answer the question. Always answer in Indonesian language. If the answer is long, try to make it to be bullet points.
  ...
  qa_system_prompt = """
  You are an assistant for question-answering tasks. Use the following pieces of retrieved context to answer the question. Always answer in Indonesian language. If the answer is long, try to make it to be bullet points.
  When creating multiple choices quiz, set the choices in bullet points and put the answer below it with explanation. Example:
  What is the purpose of the document "General Standardization Development Guideline"?
   A) Translation 
   B) Bug fixing 
